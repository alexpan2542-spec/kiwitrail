...
def test_search_items_empty_result(mock_search_items, client):
    # Mock the scenario where no results are found
    mock_search_items.return_value = []
...
def test_search_gazetteer_success(mock_gazetteer_search, client):
    # Mock the result returned by the in-memory place name matching
    mock_gazetteer_search.return_value = [
        {"name": "Wellington", "lat": -41.2865, "lng": 174.7762}
    ]

    # Use the fuzzy_search field to trigger the fuzzy search logic
    payload = {
...
def test_search_gazetteer_validation_error(mock_gazetteer_search, client):
    # Test Pydantic Schema validation (e.g., limit exceeds the le=1000 constraint)
    payload = {
        "fuzzy_search": "Auckland",
        "limit": 9999  # TrackSearchRequest defines le=1000
    }

    response = client.post("/search/gazetteer", json=payload)

    # FastAPI will automatically intercept invalid requests using Pydantic and return a 422 error
